...
def select_bracket(v):
    ...
    def matched(region):
        ...
        if is_matched_brackets(lb, rb):
            return BracketRegion(lb, sublime.Region(a, a + 1),
...
class BracketsTransCommand(sublime_plugin.TextCommand):

    def run(self, edit):
        def replace_bracket(br):
            for b in BRACKETS:
                if b != br.shape:
                    v.replace(edit, br.left, b)
                    v.replace(edit, br.right, BRACKETS[b])
                    return
